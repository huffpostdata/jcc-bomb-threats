...
module.exports = class PageContext {
  ...
  path_to_asset(key) {
    return this.assets.hrefTo(key)
  }

  asset_contents(type, key) {
    return this.assets.dataFor(key)
  }
   ...
   */
  path_to(key) {
    const path = key === '/' ? this.compiler.base_path : `${this.compiler.base_path}${key}`
