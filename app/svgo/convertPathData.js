'use strict';
// Copied from https://raw.githubusercontent.com/svg/svgo/master/plugins/convertPathData.js
// Nixed applyTransforms
// Renamed ../lib/svgo/tools => ./tools
// Changed calling convention: take string as input, give string as output
 ...
 * @author Kir Belevich
 */
exports.fn = function(d, params) {
    const item = {
      value: d,

      attr(k) {
        if (k === 'd') return this // so path.attr('d').value is r/w
        throw new Error(`Expected caller to only use attr('d')`)
      },

      hasAttr(k) { return false }
    }
